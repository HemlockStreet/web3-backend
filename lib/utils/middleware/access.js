...
module.exports = class AccessMiddleware {
  ...
  atkn(req, res, next) {
    ...
    const decoded = this.auth.tkn.utils.verify('atkn', atkn);
    if (!decoded) {
      this.auth.tkn.logUserOut(req.userData.address);
      return rejection('atkn', 'expired', res);
    }
    if (req.ip !== decoded.ip) return rejectAs('invalid');

    // goto next
    req.userData.tier = this.auth.tkn.roles.scopeTier(req.userData.scope);
    next();
  }
