...
module.exports = class LoginMiddleware {
  ...
  ctknSig(req, res, next) {
    ...
    // validate ctkn
    const ip = req.ip;
    const decoded = this.auth.tkn.utils.verify('atkn', message);
  ...
  rtkn(req, res, next) {
    ...
    const decoded = this.auth.tkn.utils.verify('rtkn', rtkn);
    if (!decoded) return rejectAs('expired');
    const { address, ip } = decoded;
    if (!this.auth.tkn.isLoggedIn(address)) return rejectAs('expired');
    ...
    req.userData = {
      ...
      ip,
      timestamp: new Date(),
      scope: this.auth.tkn.roles.scopeOf(address),
      roles: this.auth.tkn.roles.rolesOf(address),
    };
    next();
  }
};
