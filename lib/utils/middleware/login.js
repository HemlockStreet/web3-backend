...
module.exports = class LoginMiddleware {
  ...
  ctknSig(req, res, next) {
    ...
    // validate ctkn
    const ip = req.ip;
    const decoded = this.auth.manager.tkn.verify('atkn', message);
  ...
  rtkn(req, res, next) {
    ...
    const decoded = this.auth.manager.tkn.verify('rtkn', rtkn);
    if (!decoded) return rejectAs('expired');
    const { address, ip } = decoded;
    if (!this.auth.manager.isLoggedIn(address)) return rejectAs('expired');
    ...
    req.userData = {
      ...
      ip,
      timestamp: new Date(),
      scope: this.auth.manager.roles.scopeOf(address),
      roles: this.auth.manager.roles.rolesOf(address),
    };
    next();
  }
};
